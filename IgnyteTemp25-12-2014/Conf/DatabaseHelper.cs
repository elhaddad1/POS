				...
				try
				{
					if (configFileAndPath.Trim().Length == 0)
					{
						...
						if (!System.IO.File.Exists(configFileAndPath))
						{
							...
							configFileAndPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory + "bin", "DAPOS.config");
							if (File.Exists(configFileAndPath) == false)
								configFileAndPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DAPOS.config");
